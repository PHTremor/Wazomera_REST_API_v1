...
import (
	...
	"wazomera/app/models"
	"wazomera/core/utils"
	"wazomera/platform/database"

	"github.com/gofiber/fiber/v2"
	"github.com/google/uuid"
)

// GetBooks func gets all existing books.
// GetBooks func gets all existing books.
...
func GetBook(c *fiber.Ctx) error {
	...
	if err != nil {
		...
		return c.Status(fiber.StatusNotFound).JSON(fiber.Map{
			"error": true,
			"msg":   "Book with the given ID was not found",
			"book":  nil,
		})
	}
...
func CreateBook(c *fiber.Ctx) error {
	...
	// Checking... if now - current time - is greather than expiration, from JWT.
	if now > expires {
	...
	// If not...
	//  Create an instance of Book struct
	book := &models.Book{}
...
// UpdateBook func - updates book by given ID.
...
func UpdateBook(c *fiber.Ctx) error {
	...
	// Checking, if current time, now, is greather than expiration from JWT.
	if now > expires {
		// Return status 401 and unauthorized error message.
		return c.Status(fiber.StatusUnauthorized).JSON(fiber.Map{
			"error": true,
...
// DeleteBook func - deletes book by given ID.
...
func DeleteBook(c *fiber.Ctx) error {
	...
	// Checking, if book with given ID exists.
	foundedBook, err := db.GetBook(book.ID)
	if err != nil {
		// Return status 404 and book not found error.
		return c.Status(fiber.StatusNotFound).JSON(fiber.Map{
